...
show tables;
create table city(ID int,NAME varchar(50),COUNTRYCODE varchar(50),DISTRICT varchar(30),
...
create table STATION(ID int,City varchar(50),state varchar(50),Lat_N int,Long_W int);
...
create table product(product_id int, product_name varchar(25), unit_price int, primary key(product_id));
create table sales(
...
-- Q26. Write an SQL query to get the names of products that have at least 100 units ordered in 
...
-- Q28. Write an SQL query to report the customer_id 
...
-- Q37. Write an SQL query to show the unique ID of each user, 
...
-- Q40. Write an SQL query to find the average selling price for each product. 
-- Average_price should be rounded to 2 decimal places.
...
-- Q45. Write an SQL query to report the respective department name and 
...
-- Q46. Write an SQL query to report the customer ids from the 
...
-- Q50. Write a SQL query to find the highest grade with its corresponding course for each student. 
-- In case of a tie, you should find the course with the smallest course_id.
drop table Players;
